...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='',val_flag=False):
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        # img = OneToThree2_7_7(img)
        # img = OneToThreeCLAHE(img)
        # img = OneToThreeUSM(img)
        # img = OneToThreeEqualizeHist(img)
        # img = OneToThreeGaussianBlur(img)
        # img = receptiveField(img)
        # img = OneToThree2(img)
        # img = OneToThree3(img)
        # img = OneToThree2_ADD_RB_CVTcolor(img)
        # img = OneToThree2_ADD_RB_npadd_CVTcolor(img)
        # img = OneToThree2_ADD_RB(img)
        # img = OneToThree2_ADD_RB_FAST(img)
        # img = OneToThree2_ORDER_BR(img)
        # img = OneToThree2_ORDER_RB(img)
        # img = OneToThreeCLAHE_THEN_EqualizeHist(img)
        # img = OneToThreeEqualizeHist_THEN_CLAHE(img)
        # img = OneToThree_EqualizeHist(img)
        # img = OneToThreeEqualizeHist_ADD_CLAHE(img)
        # img = OneToThreeEqualizeHist_ADD_CLAHE_FAST(img)

        # img = OneToThreeEqualizeHist_Muti_CLAHE(img)
        # img = OneToThree_CLAHE(img)
        # img = OneToThree_receptiveField(img)
        # img = OneToThree_CLAHE(img)
        # img = OneToThree_Sharpen(img)

        # img = OneToThreeGaussianBlur3_3(img)
        # img = OneToThreeGaussianBlur5_5(img)
        # img = OneToThreeGaussianBlur7_7(img)
        #
        # img = OneToThree2_MeanBlur3_3(img)
        # img = OneToThree2_MeanBlur5_5(img)
        # img = OneToThree2_MeanBlur7_7(img)
        #
        # img = OneToThree2_MedianBlur3_3(img)
        # img = OneToThree2_MedianBlur5_5(img)
        # img = OneToThree2_MedianBlur7_7(img)
        # img = OneToThree2_Gamma(img)
        img = OneToThree2_Log(img)

        # img = OneToThree2_ORDER_RB(img)
        # img = OneToThree_CLAHE_USM_P(img)

        t2 = time_sync()

        self.time_d += t2 - t1
        if self.count >= self.nf:
            s = f'ms: {self.time_d / self.count * 1000} '
            print(s)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',val_flag=False):
        self.img_size = img_size
